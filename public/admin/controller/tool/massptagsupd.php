...
class ControllerToolmassptagsupd extends Controller { 
	...
	public function index() {
		...
		// tags updates
		$this->data['masstxt_p_tags_updates'] = $this->language->get('masstxt_p_tags_updates');
		$this->data['masstxt_new_tags'] = $this->language->get('masstxt_new_tags');
		$this->data['masstxt_new_tags_help'] = $this->language->get('masstxt_new_tags_help');
		...
		if (isset($this->request->post['tags_upd'])) {
			$this->data['tags_upd'] = $this->request->post['tags_upd'];
		} else {
			$tags_upd = array();
			foreach ($languages as $language) { $tags_upd[$language['language_id']] = ''; }
			$this->data['tags_upd'] = $tags_upd;
		}
...
switch ($this->request->post['upd_mode']) { /// update mode
	...
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
		if ($tags!="") {
			...
			$arr_new_tags = array();
			$arr_new_tags = explode(',', $tags);
				foreach ($arr_new_tags as $tag) {
	...
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_tag WHERE product_id = '" . (int)$product_id. "' AND language_id = '" . (int)$language_id . "'");
		$arr_new_tags = array();
		$arr_new_tags = explode(',', $tags);
			foreach ($arr_new_tags as $tag) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_tag SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', tag = '" . $this->db->escape(trim($tag)) . "'");
	...
	foreach ($this->request->post['tags_upd'] as $language_id => $tags) {
		if ($tags!="") {
			...
			$arr_new_tags = explode(',', $tags);
				foreach ($arr_new_tags as $tag) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_tag WHERE product_id = '" . (int)$product_id. "' AND language_id = '" . (int)$language_id . "' AND tag = '" . $this->db->escape(trim($tag)) . "'");
				}
			}
		}
	}
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		'href'      => $this->url->link('tool/massptagsupd', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('tool/massptagsupd', 'token=' . $this->session->data['token'], 'SSL');

		$this->template = 'tool/massptagsupd.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
