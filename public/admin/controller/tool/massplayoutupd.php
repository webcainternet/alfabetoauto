...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		'href'      => $this->url->link('tool/massplayoutupd', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('tool/massplayoutupd', 'token=' . $this->session->data['token'], 'SSL');

		$this->template = 'tool/massplayoutupd.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
