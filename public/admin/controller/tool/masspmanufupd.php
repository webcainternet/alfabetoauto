...
class ControllerToolmasspmanufupd extends Controller { 
	...
	public function index() {
		...
		$this->data['masstxt_new_manufacturer'] = $this->language->get('masstxt_new_manufacturer');
		...
		if (isset($this->request->post['filter_attr'])) {
    		...
    		if (isset($this->request->post['manufacturer_id_upd'])) {
      			$this->data['manufacturer_id_upd'] = $this->request->post['manufacturer_id_upd'];
		} else {
      			$this->data['manufacturer_id_upd'] = 0;
    		}
...
$this->session->data['success'] = $this->language->get('masstxt_succes_products_filtered');
		...
		'href'      => $this->url->link('tool/masspmanufupd', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('tool/masspmanufupd', 'token=' . $this->session->data['token'], 'SSL');

		$this->template = 'tool/masspmanufupd.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
